/**
* mobilegateway API generated from mobilegateway.yang
*
* NOTE: This file is auto generated by polycube-codegen
* https://github.com/polycube-network/polycube-codegen
*/


/* Do not edit this file manually */

/*
* PortsJsonObject.h
*
*
*/

#pragma once


#include "JsonObjectBase.h"

#include "PortsSecondaryipJsonObject.h"
#include <vector>

namespace polycube {
namespace service {
namespace model {

enum class PortsDirectionEnum {
  UE, PDN
};

/// <summary>
///
/// </summary>
class  PortsJsonObject : public JsonObjectBase {
public:
  PortsJsonObject();
  PortsJsonObject(const nlohmann::json &json);
  ~PortsJsonObject() final = default;
  nlohmann::json toJson() const final;


  /// <summary>
  /// Port Name
  /// </summary>
  std::string getName() const;
  void setName(std::string value);
  bool nameIsSet() const;

  /// <summary>
  /// IP address and prefix of the port
  /// </summary>
  std::string getIp() const;
  void setIp(std::string value);
  bool ipIsSet() const;
  void unsetIp();

  /// <summary>
  /// Additional IP addresses for the port
  /// </summary>
  const std::vector<PortsSecondaryipJsonObject>& getSecondaryip() const;
  void addPortsSecondaryip(PortsSecondaryipJsonObject value);
  bool secondaryipIsSet() const;
  void unsetSecondaryip();

  /// <summary>
  /// MAC address of the port
  /// </summary>
  std::string getMac() const;
  void setMac(std::string value);
  bool macIsSet() const;
  void unsetMac();

  /// <summary>
  /// Whether the port faces User Equipments (UE) or an external Packet Data Network (PDN)
  /// </summary>
  PortsDirectionEnum getDirection() const;
  void setDirection(PortsDirectionEnum value);
  bool directionIsSet() const;
  void unsetDirection();
  static std::string PortsDirectionEnum_to_string(const PortsDirectionEnum &value);
  static PortsDirectionEnum string_to_PortsDirectionEnum(const std::string &str);

private:
  std::string m_name;
  bool m_nameIsSet;
  std::string m_ip;
  bool m_ipIsSet;
  std::vector<PortsSecondaryipJsonObject> m_secondaryip;
  bool m_secondaryipIsSet;
  std::string m_mac;
  bool m_macIsSet;
  PortsDirectionEnum m_direction;
  bool m_directionIsSet;
};

}
}
}

