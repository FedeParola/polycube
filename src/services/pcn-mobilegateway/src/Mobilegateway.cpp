...
Mobilegateway::Mobilegateway(const std::string name, const MobilegatewayJsonObject &conf)
  : Cube(conf.getBase(), { mobilegateway_code }, {}),
    MobilegatewayBase(name),
    netlink_instance_router_(polycube::polycubed::Netlink::getInstance()) {
  logger()->info("Creating Mobilegateway instance");
  ...
  if (get_shadow()) {
    ...
    netlink_notification_index_route_added = netlink_instance_router_.registerObserver(
      ...
      std::bind(&Mobilegateway::netlink_notification_route_added, this,
    ...
    netlink_notification_index_route_deleted = netlink_instance_router_.registerObserver(
      ...
      std::bind(&Mobilegateway::netlink_notification_route_deleted, this,
                std::placeholders::_1, std::placeholders::_2));
    ...
    netlink_notification_index_link_deleted = netlink_instance_router_.registerObserver(
      ...
      std::bind(&Mobilegateway::netlink_notification_link_deleted, this,
                std::placeholders::_1, std::placeholders::_2));
    ...
    netlink_notification_index_new_address = netlink_instance_router_.registerObserver(
      ...
      std::bind(&Mobilegateway::netlink_notification_new_address, this,
                std::placeholders::_1, std::placeholders::_2));
  }
}
...
void Mobilegateway::packet_in(Ports &port,
...
std::shared_ptr<Ports> Mobilegateway::getPorts(const std::string &name) {
  // call default implementation in base class
  return MobilegatewayBase::getPorts(name);
}

// Basic default implementation, place your extension here (if needed)
std::vector<std::shared_ptr<Ports>> Mobilegateway::getPortsList() {
  // call default implementation in base class
  return MobilegatewayBase::getPortsList();
}

// Basic default implementation, place your extension here (if needed)
void Mobilegateway::addPorts(const std::string &name, const PortsJsonObject &conf) {
  MobilegatewayBase::addPorts(name, conf);
  if (get_shadow()) {
...
// Basic default implementation, place your extension here (if needed)
void Mobilegateway::addPortsList(const std::vector<PortsJsonObject> &conf) {
  // call default implementation in base class
  MobilegatewayBase::addPortsList(conf);
}

// Basic default implementation, place your extension here (if needed)
void Mobilegateway::replacePorts(const std::string &name,
                          const PortsJsonObject &conf) {
  // call default implementation in base class
  MobilegatewayBase::replacePorts(name, conf);
}

// Basic default implementation, place your extension here (if needed)
void Mobilegateway::delPorts(const std::string &name) {
  ...
  // call default implementation in base class
  MobilegatewayBase::delPorts(name);

  logger()->info("Port {0} was removed", name);
}

// Basic default implementation, place your extension here (if needed)
void Mobilegateway::delPortsList() {
  // call default implementation in base class
  MobilegatewayBase::delPortsList();
}

std::shared_ptr<Route> Mobilegateway::getRoute(const std::string &network,
...
std::vector<std::shared_ptr<Route>> Mobilegateway::getRouteList() {
...
void Mobilegateway::addRoute(const std::string &network,
                      const std::string &nexthop,
...
// Basic default implementation, place your extension here (if needed)
void Mobilegateway::addRouteList(const std::vector<RouteJsonObject> &conf) {
  // call default implementation in base class
  MobilegatewayBase::addRouteList(conf);
}

// Basic default implementation, place your extension here (if needed)
void Mobilegateway::replaceRoute(const std::string &network,
                          const std::string &nexthop,
                          const RouteJsonObject &conf) {
  // call default implementation in base class
  MobilegatewayBase::replaceRoute(network, nexthop, conf);
}

void Mobilegateway::delRoute(const std::string &network, const std::string &nexthop) {
  logger()->debug(
...
// Basic default implementation, place your extension here (if needed)
void Mobilegateway::delRouteList() {
  // call default implementation in base class
  MobilegatewayBase::delRouteList();
}

std::shared_ptr<ArpTable> Mobilegateway::getArpTable(const std::string &address) {
...
std::vector<std::shared_ptr<ArpTable>> Mobilegateway::getArpTableList() {
...
void Mobilegateway::addArpTable(const std::string &address,
...
// Basic default implementation, place your extension here (if needed)
void Mobilegateway::addArpTableList(const std::vector<ArpTableJsonObject> &conf) {
  // call default implementation in base class
  MobilegatewayBase::addArpTableList(conf);
}

// Basic default implementation, place your extension here (if needed)
void Mobilegateway::replaceArpTable(const std::string &address,
                             const ArpTableJsonObject &conf) {
  // call default implementation in base class
  MobilegatewayBase::replaceArpTable(address, conf);
}

void Mobilegateway::delArpTable(const std::string &address) {
...
// Basic default implementation, place your extension here (if needed)
void Mobilegateway::delArpTableList() {
  // call default implementation in base class
  MobilegatewayBase::delArpTableList();
}

std::shared_ptr<UserEquipment> Mobilegateway::getUserEquipment(const std::string &ip) {
  return std::shared_ptr<UserEquipment>(&user_equipments_.at(ip), [](UserEquipment *) {});
}

std::vector<std::shared_ptr<UserEquipment>> Mobilegateway::getUserEquipmentList() {
  std::vector<std::shared_ptr<UserEquipment>> ue_vect;
  for (auto &it : user_equipments_) {
    ue_vect.push_back(std::shared_ptr<UserEquipment>(&it.second, [](UserEquipment *) {}));
  }
  
  return ue_vect;
}

void Mobilegateway::addUserEquipment(const std::string &ip, const UserEquipmentJsonObject &conf) {
  if (user_equipments_.count(ip) > 0) {
    throw std::runtime_error("User Equipment already exists");
  }
  user_equipments_.emplace(std::piecewise_construct, std::forward_as_tuple(ip),
                           std::forward_as_tuple(*this, conf));
  logger()->info("Added User Equipment [ip: {0}]", conf.getIp());
}

// Basic default implementation, place your extension here (if needed)
void Mobilegateway::addUserEquipmentList(const std::vector<UserEquipmentJsonObject> &conf) {
  // call default implementation in base class
  MobilegatewayBase::addUserEquipmentList(conf);
}

// Basic default implementation, place your extension here (if needed)
void Mobilegateway::replaceUserEquipment(const std::string &ip, const UserEquipmentJsonObject &conf) {
  // call default implementation in base class
  MobilegatewayBase::replaceUserEquipment(ip, conf);
}

void Mobilegateway::delUserEquipment(const std::string &ip) {
  if (user_equipments_.count(ip) == 0) {
    throw std::runtime_error("User Equipment does not exist");
  }
  user_equipments_.erase(ip);
  logger()->info("Removed User Equipment [ip: {0}]", ip);
}

// Basic default implementation, place your extension here (if needed)
void Mobilegateway::delUserEquipmentList() {
  // call default implementation in base class
  MobilegatewayBase::delUserEquipmentList();
}

std::shared_ptr<BaseStation> Mobilegateway::getBaseStation(const std::string &ip) {
  return std::shared_ptr<BaseStation>(&base_stations_.at(ip), [](BaseStation *) {});
}

std::vector<std::shared_ptr<BaseStation>> Mobilegateway::getBaseStationList() {
  std::vector<std::shared_ptr<BaseStation>> bs_vect;
  for (auto &it : base_stations_) {
    bs_vect.push_back(std::shared_ptr<BaseStation>(&it.second, [](BaseStation *) {}));
  }
  
  return bs_vect;
}

void Mobilegateway::addBaseStation(const std::string &ip, const BaseStationJsonObject &conf) {
  if (base_stations_.count(ip) > 0) {
    throw std::runtime_error("Base Station already exists");
  }
  base_stations_.emplace(std::piecewise_construct, std::forward_as_tuple(ip),
                         std::forward_as_tuple(*this, conf));
  logger()->info("Added Base Station [ip: {0}]", conf.getIp());
}

// Basic default implementation, place your extension here (if needed)
void Mobilegateway::addBaseStationList(const std::vector<BaseStationJsonObject> &conf) {
  // call default implementation in base class
  MobilegatewayBase::addBaseStationList(conf);
}

// Basic default implementation, place your extension here (if needed)
void Mobilegateway::replaceBaseStation(const std::string &ip, const BaseStationJsonObject &conf) {
  // call default implementation in base class
  MobilegatewayBase::replaceBaseStation(ip, conf);
}

void Mobilegateway::delBaseStation(const std::string &ip) {
  if (base_stations_.count(ip) == 0) {
    throw std::runtime_error("Base Station does not exist");
  }
  // TODO: Remove all UEs that use this BS
  base_stations_.erase(ip);
  logger()->info("Removed Base Station [ip: {0}]", ip);
}

// Basic default implementation, place your extension here (if needed)
void Mobilegateway::delBaseStationList() {
  // call default implementation in base class
  MobilegatewayBase::delBaseStationList();
}
...
*/
std::string Mobilegateway::search_interface_from_nexthop(const std::string &nexthop) {
...
*/
void Mobilegateway::add_active_nexthop_in_ebpf_map(const std::string &network,
                                    const std::string &nexthop,
...
*/
void Mobilegateway::remove_route(const std::string &network,
                          const std::string &nexthop) {
  std::tuple<string, string> key(network, nexthop);
...
*/
void Mobilegateway::remove_all_routes() {
  if (routes_.size() == 0)
...
*/
void Mobilegateway::add_local_route(const std::string &interface_ip,
...
*/
void Mobilegateway::remove_local_route(const std::string &interface_ip,
                                const std::string &port_name) {
  std::string ip_route;
  std::string netmask_route;
  split_ip_and_prefix(interface_ip, ip_route, netmask_route);
...
*/
void Mobilegateway::find_new_active_nexthop(const std::string &network,
                                     const std::string &nexthop) {
  std::string ip_route;
  std::string netmask_route;
  split_ip_and_prefix(network, ip_route, netmask_route);
...
void Mobilegateway::handle_router_pkt(Port &port, PacketInMetadata &md,
                               const std::vector<uint8_t> &packet) {
...
EthernetII Mobilegateway::make_echo_reply(EthernetII &origin,
...
void Mobilegateway::generate_icmp_ttlexceed(Port &port, PacketInMetadata &md,
                                     const std::vector<uint8_t> &packet) {
...
void Mobilegateway::generate_arp_request(Port &port, PacketInMetadata &md,
                                  const std::vector<uint8_t> &packet) {
...
void Mobilegateway::generate_arp_reply(Port &port, PacketInMetadata &md,
                                const std::vector<uint8_t> &packet) {
...
/* Netlink */
void Mobilegateway::netlink_notification_route_added(int ifindex, const std::string &info_route) {
...
void Mobilegateway::netlink_notification_route_deleted(int ifindex, const std::string &info_route) {
  std::lock_guard<std::mutex> guard(router_mutex);
...
void Mobilegateway::netlink_notification_link_deleted(int ifindex, const std::string &iface) {
  std::lock_guard<std::mutex> guard(router_mutex);
...
void Mobilegateway::netlink_notification_new_address(int ifindex, const std::string &new_ip) {
  std::lock_guard<std::mutex> guard(router_mutex);
...
void Mobilegateway::add_linux_route(const std::string &network,
...
void Mobilegateway::remove_linux_route(const std::string &network,
                                const std::string &prefix,
                                const std::string &nexthop,
                                const std::string &port_name) {
  uint32_t prefix_ = std::stoi(prefix);
