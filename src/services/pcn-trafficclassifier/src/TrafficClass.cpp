...
TrafficClass::TrafficClass(Trafficclassifier &parent,
  ...
  logger()->info("Traffic class created {0}", toString());
}

TrafficClass::~TrafficClass() {
  logger()->info("Traffic class deleted {0}", toString());
}
...
void TrafficClass::setPriority(const uint32_t &value) {
  ...
  parent_.updateClassificationPipeline();
}
